...
$user = Auth::user();
$usuario = [
    'nombre'    => $user ? $user->name : 'Carlos Pérez',
    'email'     => $user ? $user->email : 'carlos@pichanga.app',
    'telefono'  => '+51 987 654 321',
    'ciudad'    => 'Cusco, Perú',
    'miembro'   => $user && $user->created_at ? $user->created_at->format('M Y') : 'Marzo 2024',
    'nivel'     => 'Amateur',
    'partidos'  => 34,
    'canchas'   => 12,
    'horas'     => 68,
...
$reservas_recientes = [
    ['id' => 1, 'cancha' => 'Complejo Deportivo El 10', 'fecha' => 'Hoy · 20:00', 'estado' => 'confirmada', 'precio' => 80.00, 'img' => 'https://images.unsplash.com/photo-1575361204480-aadea25e6e68?w=100&q=80'],
    ['id' => 2, 'cancha' => 'El Rey del Gras',          'fecha' => 'Ayer · 18:00', 'estado' => 'completada', 'precio' => 90.00, 'img' => 'https://images.unsplash.com/photo-1551280857-2b9ebf262c62?w=100&q=80'],
    ['id' => 3, 'cancha' => 'Canchas La Losa',          'fecha' => '28 Abr · 19:00', 'estado' => 'completada', 'precio' => 60.00, 'img' => 'https://images.unsplash.com/photo-1518605368461-1ee7e53f191b?w=100&q=80'],
];
@endphp
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 relative z-10">
            ...
            <div class="relative shrink-0 group cursor-pointer">
                <div class="w-28 h-28 rounded-3xl overflow-hidden ring-4 ring-brand-500/20 shadow-xl group-hover:ring-brand-500/50 transition-all duration-300">
                    <img src="https://ui-avatars.com/api/?name={{ urlencode($usuario['nombre']) }}&background=22c55e&color=fff&size=200"
            ...
            <div class="flex-1 text-center sm:text-left mt-2 sm:mt-0">
                <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-2">
                    <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ $usuario['nombre'] }}</h1>
                    <span class="inline-flex items-center gap-1.5 bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400 text-xs font-bold px-3 py-1.5 rounded-xl border border-brand-200 dark:border-brand-500/20 shadow-sm w-max mx-auto sm:mx-0">
                        <i class="ph-fill ph-shield-check text-sm"></i> {{ $usuario['nivel'] }}
                    </span>
                </div>
                <p class="text-slate-500 dark:text-slate-400 text-sm mb-6 font-medium">
                    <i class="ph-fill ph-map-pin text-slate-400 dark:text-slate-500 mr-1"></i>{{ $usuario['ciudad'] }}
                    <span class="mx-2 text-slate-300 dark:text-slate-600">·</span>
                    <i class="ph-fill ph-calendar-blank text-slate-400 dark:text-slate-500 mr-1"></i>Miembro desde {{ $usuario['miembro'] }}
                </p>

                {{-- Stats --}}
                <div class="flex justify-center sm:justify-start gap-8">
                    <div class="text-center group">
                        <div class="text-2xl font-black text-slate-900 dark:text-white group-hover:text-brand-500 transition-colors">{{ $usuario['partidos'] }}</div>
                    ...
                    <div class="text-center group">
                        <div class="text-2xl font-black text-slate-900 dark:text-white group-hover:text-blue-500 transition-colors">{{ $usuario['canchas'] }}</div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Canchas</div>
                    </div>
                    <div class="w-px bg-slate-200 dark:bg-white/10"></div>
                    <div class="text-center group">
                        <div class="text-2xl font-black text-slate-900 dark:text-white group-hover:text-purple-500 transition-colors">{{ $usuario['horas'] }}h</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="glass-card rounded-3xl p-6 md:p-8 space-y-4">
            ...
            <div class="space-y-1">
                @foreach([
                    ['icon' => 'ph-user',         'label' => 'Nombre Completo', 'value' => $usuario['nombre']],
                    ['icon' => 'ph-envelope',      'label' => 'Correo Electrónico', 'value' => $usuario['email']],
                    ['icon' => 'ph-phone',         'label' => 'Número Telefónico', 'value' => $usuario['telefono']],
                ] as $campo)
        ...
        <div class="glass-card rounded-3xl p-6 md:p-8 flex flex-col">
            ...
            <div class="space-y-3 flex-1">
                ...
                <a href="/reservas/{{ $reserva['id'] }}" class="flex items-center gap-4 p-3 rounded-2xl bg-white/50 dark:bg-dark-800/50 hover:bg-white dark:hover:bg-dark-800 border border-slate-100 dark:border-white/5 hover:border-brand-200 dark:hover:border-brand-500/30 transition-all cursor-pointer shadow-sm hover:shadow-md dark:shadow-none group">
                    <div class="w-12 h-12 rounded-xl overflow-hidden shrink-0 shadow-sm relative">
                        <img src="{{ $reserva['img'] }}" class="w-full h-full object-cover" alt="Cancha">
                        <div class="absolute inset-0 bg-black/10 group-hover:bg-transparent transition-colors"></div>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-bold text-slate-900 dark:text-white truncate group-hover:text-brand-500 transition-colors">{{ $reserva['cancha'] }}</p>
                        <p class="text-[11px] font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1 mt-1">
                            <i class="ph-fill ph-clock"></i> {{ $reserva['fecha'] }}
                        </p>
                    </div>
                    <div class="text-right shrink-0">
                        <p class="text-sm font-black text-slate-900 dark:text-white mb-1">S/ {{ number_format($reserva['precio'], 2) }}</p>
